...
-- crée leur ligne profiles (is_bot, Elo fixe) et renseigne bot_roster.profile_id.
-- Ce script reste donc PUREMENT additif et sans risque (aucun write sur profiles).
--
-- CE QUE FAIT CE SCRIPT (idempotent) :
--   1. table bot_roster (métadonnées : nom, rang, Elo, tier, lore) + lecture
--      publique ; profile_id NULL tant que le worker n'a pas provisionné ;
--   2. matchmaking_queue : 2 colonnes additives (want_backfill, backfill_after)
--      — défauts sûrs, l'insert existant de l'app n'est pas impacté ;
--   3. suivi des rencontres (bot_encounters) + RPC mark/get pour la galerie ;
--   4. pg_cron d'entretien : garde les bots provisionnés « en ligne » + fauche
--      les entrées de file de bots périmées (anti-partie-orpheline).
--
-- ⚠ EXCLUSION CLASSEMENT (à faire côté app, lot 3) : le leaderboard interroge
--   profiles directement → filtrer is_bot, sinon un bot Elo 2800 trusterait le
--   top. La ligue est déjà protégée (dev_bot_army.sql).
--
-- DÉCISION PRODUIT : parties de backfill AMICALES (ranked=false) côté worker,
--   pour ne pas farmer l'Elo contre des bots à Elo fixe. Flippable.
-- ══════════════════════════════════════════════════════════════════

-- ── 1. Table roster + lecture publique ────────────────────────────
-- profile_id NULLABLE : rempli par le worker une fois le compte auth créé.
create table if not exists bot_roster (
  ...
  pseudo     text not null,          -- pseudo ASCII du profil (^[A-Za-z0-9_-]{3,20}$)
...
-- Si une version antérieure a créé la table avec profile_id NOT NULL / sans
-- pseudo, on rattrape (idempotent).
...
-- ── 2. Seed roster (métadonnées uniquement — pas de write sur profiles) ─
-- pseudo ASCII prêt pour le worker (contrainte profiles.pseudo ^[A-Za-z0-9_-]{3,20}$).
insert into bot_roster(key,num,reading,rank,pseudo,kanji,rank_kanji,meaning,lore,base_elo,tier,sort) values
...
on conflict (key) do update set
  ...
  lore=excluded.lore, base_elo=excluded.base_elo, tier=excluded.tier, sort=excluded.sort;
...
begin
  -- garde présents les bots déjà provisionnés (profile_id non NULL)
