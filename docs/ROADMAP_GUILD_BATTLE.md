...
| **G5** | ✅ **LIVRÉ 2026-08-17.** **Arbre Tekken + spectateur** : rendu SVG complet avec flèches, bandeau de série, combat en cours cliquable. | G4, B, C |
| **G6** | ✅ **LIVRÉ 2026-08-17.** **Confrontation amicale** : réutilise G2→G5 avec deux guildes. Déclaration, acceptation du GM adverse, planification à 48 h max. Aucun enjeu. | G5, T |
| **G7** | ✅ **LIVRÉ 2026-08-17.** **Classement des guildes** : colonne `guild_elo`, refonte de `v_guild_leaderboard` et de l'écran « Autres guildes » (le Ryu y devient trésorerie, plus rang). Branchement sur le Défi guilde 48 h existant. | G6, AE, AF, AK |
| **G8** | ✅ **LIVRÉ 2026-08-18** (avec une limitation connue, voir § 10). **Attaque de guilde** : ciblage par tranche de classement, cooldowns, vol de 10 🐉, forfait, annulation, effet sur l'Elo de guilde, notifications d'attaque. | G7, AG, Q, R, V |
...
### G5 — Arbre Tekken + spectateur (2026-08-17)
- Aucun SQL nouveau : lot purement client, réutilise `guild_event_state` (déjà exposé par G4 : `current_match`, `matches`).
- `index.html` : `buildGuildEventTreeSvg(teamA, teamB, matches, currentMatch)` (nouvelle fonction globale) + `openGuildEventRoster()` modifié pour l'appeler à la place des blocs d'équipe plats dès que `status` est `running`/`finished`.
- Principe du rendu : la position (équipe + siège) de chaque joueur est fixe pour tout le tournoi (décidée à l'inscription/répartition, jamais réordonnée). Une flèche part donc simplement de la position du perdant vers la position du vainqueur pour chaque match clos (`winner is not null`) — plusieurs flèches convergeant sur un même nœud dessinent naturellement une série de victoires, sans état de « streak » à recalculer côté rendu.
- Nœud en cours de combat : liseré doré, cliquable, appelle `spectateGame(currentMatch.game_id)` (patron repris identique à `joinTournamentGame()`/G4 — aucune nouvelle mécanique de spectateur). Nœud éliminé : grisé + ✕.
- Vérifié en navigateur (serveur statique local, données mock injectées via la console) : champion avec flèches de série convergentes, éliminés grisés, duel en cours en surbrillance et cliquable — capture d'écran conforme à la référence Tekken Team Battle.
- Aucune nouvelle clé i18n (le SVG n'a pas de texte traduit — pseudos + symboles ✕/⚔ uniquement).

### G6 — Confrontation amicale (2026-08-17)
- `sql_a_executer/guild_events_g6_friendly.sql` (exécuté via MCP) : ajoute `pending_acceptance` au check constraint de `guild_events.status` ; `guild_event_propose_friendly`/`guild_event_respond_friendly`/`guild_event_pending_for_me` (nouvelles RPC) ; réécrit `guild_event_register` (équipe fixée par appartenance de guilde pour `kind<>'internal'`, au lieu de rester `null` en attendant l'autobalance du GM), `guild_event_cancel` (chef de N'IMPORTE LAQUELLE des deux guildes), `guild_event_list_mine` (inclut aussi les événements où ma guilde est `guild_b`), `guild_events_registration_tick` (ordonne les sièges par Elo automatiquement à la clôture pour `kind<>'internal'`, puisqu'il n'y a pas d'étape d'autobalance manuelle côté amical/attaque), `guild_events_checkin_tick` (remplace le simple test d'existence d'équipe par un comptage `>=3` par camp — décision O, applicable aussi au Tournoi interne comme filet de sécurité), `guild_event_state` (ajoute `kind`/`guild_a`/`guild_b`/`guild_a_name`/`guild_b_name`, conserve `current_match`/`matches` du lot G4).
- `index.html` : bouton « 🤝 Confrontation amicale » dans `renderMyGuild()`, écran `openGuildFriendlyMatches()` (juste avant l'Arbre Tekken, ~ligne 7937) + `proposeFriendlyMatch`/`respondFriendlyMatch`/`registerFriendlyEvent`/`unregisterFriendlyEvent`/`cancelFriendlyEvent`. `openGuildEventRoster()` et `buildGuildEventTreeSvg()` généralisés (kind-aware) : bouton retour et libellés d'équipe pointent vers le nom de la guilde adverse au lieu de « ÉQUIPE A/B » quand `kind<>'internal'` (décision D — jamais confondre les trois systèmes portant le mot « guilde »).
- Notifications `guild_event_challenge_received` (proposition reçue, chef ciblé uniquement) et `guild_event_challenge_declined` (chef proposant, si refusé) câblées aux 6 points (icônes 🤝/🚫, routage clic → `guildOpenFriendlyMatchesFromNotification()`, pastille Arène). La confirmation d'acceptation réutilise `guild_event_announced` (déjà câblé) — envoyée à TOUS les membres des deux guildes.
- Aucune nouvelle clé i18n.
- Testé de bout en bout par impersonation sur deux guildes jetables (4 bots chacune, Elo variés) : proposition → notification reçue par le chef adverse uniquement → acceptation → notification aux 8 membres des deux guildes → inscription des 8 → équipe assignée automatiquement selon la guilde d'origine (aucune répartition manuelle) → clôture forcée des inscriptions → ordre de passage par Elo croissant vérifié dans les deux équipes → check-in des 8 → lancement → premier duel créé par le moteur G4 **sans aucune modification** (seat 1 vs seat 1, les deux Elo les plus faibles) — preuve que le moteur de combat est déjà kind-agnostic. Nettoyage complet effectué après test.

### G7 — Classement des guildes (2026-08-17)
- `sql_a_executer/guild_events_g7_elo.sql` (exécuté via MCP, en 3 migrations successives) : colonne `guilds.guild_elo` (défaut 1200) ; **`v_guild_leaderboard` DROP puis recréée** (ajout d'une colonne au milieu — piège CLAUDE.md, `create or replace view` aurait échoué en 42P16) triant désormais sur `guild_elo desc` (Ryu toujours affiché, mais n'influence plus le rang) ; fonction interne `guild_elo_apply_result(winner, loser, score_winner, score_loser, weight)` (`EXECUTE` révoqué pour public — appelée uniquement depuis d'autres fonctions `SECURITY DEFINER`) : formule Elo standard (même famille que `computeNewElo()` côté joueur, `index.html`), l'écart de score sert de marge de victoire (0.5 = serré, 1.0 = écrasant), `p_weight` pondère le résultat final (1.0 réservé à l'Attaque de guilde du lot G8, 0.5 utilisé dès maintenant par le Défi guilde 48h — décision AK) ; `guild_challenges_cleanup` (Défi guilde 48h, déjà en prod) réécrite pour appeler `guild_elo_apply_result(..., 0.5)` à la clôture et l'annoncer dans la notification de victoire ; `get_my_guild()` et `guild_roster()` réécrites pour exposer `guild_elo` (sinon le client ne peut afficher ni son propre Elo de guilde ni celui d'une guilde consultée).
- `index.html` : `⚔ Elo` affiché dans la liste « Autres guildes » (classement, `renderGuildList`), dans le header de « Ma guilde » (`renderMyGuild`) et dans le roster public d'une guilde consultée. Aucune nouvelle clé i18n (juste un symbole + un nombre).
- Testé isolément (guildes jetables, sans passer par un vrai Défi 48h) : `guild_elo_apply_result` avec Elo égaux (1200/1200) et score 30-5 donne bien +11/-11 en poids plein (1.0) et +6/-6 en poids moitié (0.5, le cas réellement actif aujourd'hui via le Défi guilde) — formule et pondération vérifiées avant nettoyage.
- Ce que ce lot NE fait PAS encore : rien ne fait encore bouger l'Elo à PLEIN poids (1.0) — ça n'arrivera qu'avec l'Attaque de guilde (lot G8). `guild_elo_apply_result` est déjà prête à être appelée avec `p_weight=1.0` à ce moment-là, sans aucune modification.

### G8 — Attaque de guilde (2026-08-18) — DERNIER LOT DU CHANTIER
- `sql_a_executer/guild_events_g8_attack.sql` (exécuté via MCP) : colonnes `guilds.last_attack_at`/`last_attacked_at` ; `guild_attack_candidates(elo_min, elo_max)` (même patron qu'`invasion_candidates`, exclut ma guilde, les guildes protégées par la fenêtre de 4 jours, et celles déjà engagées dans une attaque non terminée) ; `guild_attack_declare` (chef uniquement, **aucune acceptation requise contrairement à l'amical** — décision R : l'attaque se déclare, ne se négocie pas ; cooldowns attaquant 7j / défenseur 4j avec bypass admin décision AD étendue depuis l'Invasion — bypass si l'appelant OU un membre de la guilde ciblée est admin ; fenêtre 15min-48h ; le cooldown démarre à la DÉCLARATION, pas au résultat) ; `guild_attack_cancel` (attaquant seul, jusqu'à 1h avant le début, 5 🐉 transférés au défenseur, cooldown reste consommé — décisions R/W) ; `guild_event_cancel` réécrite pour rejeter `kind='attack'` (route obligatoire par `guild_attack_cancel`, règles trop différentes pour rester dans la fonction générique) ; `guild_events_checkin_tick` réécrite une 3e fois : ajoute les pénalités décision Q (défense vide → défenseur -5 🐉, attaquant +5 🐉) et décision X (effectif attaquant < 3 → même traitement qu'une annulation volontaire), avant le repli générique (O) pour les autres cas ; `guild_events_combat_tick` réécrite : résolution finale désormais **kind-aware et notifie les DEUX guildes** pour `friendly`/`attack` (corrige au passage un vrai trou du lot G6 : la version précédente ne notifiait QUE `guild_a`, avec un texte « équipe A/B » qui ne nommait jamais les guildes adverses) ; pour `kind='attack'` : vol de 10 🐉 fixes (décision AA) + appel à `guild_elo_apply_result(..., poids=1.0)` (décision AJ, marge = survivants restants dans l'équipe victorieuse).
- `index.html` : bouton « ⚔ Attaque de guilde » dans `renderMyGuild()`, écran `openGuildAttacks()` (juste avant l'Arbre Tekken) + sélecteur de tranche (à mon niveau/plus forte/plus faible, même patron que le sélecteur d'invasion) + `declareGuildAttack`/`registerAttackEvent`/`cancelAttackEvent`. `openGuildEventRoster()` étendu une 3e fois pour router vers `openGuildAttacks()`. Notifications `guild_attack_declared`/`guild_attack_cancelled` câblées aux 6 points. **Correctif transversal** : les notifications `guild_event_announced`/`registration_closed`/`starting`/`result` (partagées par les 3 systèmes internal/friendly/attack, mais dont le TYPE ne distingue pas lequel) routaient TOUJOURS vers l'écran Tournoi interne, même pour un événement amical ou une attaque — bug latent depuis le lot G6, jamais remarqué faute de l'avoir testé en navigateur. Corrigé en propageant `payload.event_id` jusqu'à `handleNotificationClick()` (nouveau paramètre) et en ajoutant `guildOpenEventByKindFromNotification()` qui relit `guild_event_state(event_id).event.kind` avant de router vers le bon écran.
- Testé de bout en bout par impersonation sur deux guildes jetables (attaquant 4 membres, défenseur 4 membres) : `guild_attack_candidates` (exclut la guilde propre), `guild_attack_declare` (cooldown attaquant 7j vérifié bloquant, cooldown défenseur 4j vérifié bloquant, bypass admin vérifié en passant outre le cooldown défenseur), verrou « busy » (une 2e attaque sur la même cible pendant que la 1re est active est refusée), combat 4v4 complet jusqu'à la victoire de l'attaquant (formule Elo vérifiée EXACTEMENT : Elo 1200 vs 1210, victoire 4 survivants/0, poids 1.0 → +16/-16, calcul à la main confirmé) + vol de 10 🐉 confirmé des deux côtés + notifications correctement adressées aux 8 membres avec le bon texte attaquant/défenseur, annulation volontaire (fenêtre 1h vérifiée bloquante puis passante, transfert de 5 🐉 confirmé), décision Q (défense vide → cancel + 5 🐉 défenseur→attaquant confirmé), décision X (attaquant < 3 → cancel + 5 🐉 attaquant→défenseur confirmé). Nettoyage complet effectué après test (guildes, is_admin de test).
- ~~🔴 Limitation connue : décision F non construite~~ **Levée le 2026-08-18, voir lot G8b ci-dessous.**
- ~~🔴 Gap transversal décision AB~~ **Décision AB annulée par Wurmz le 2026-08-18** : *« je ne vois pas de souci au fait que ça compte pour une victoire, une victoire est une victoire. »* Les combats de guilde (Tournoi interne, Confrontation amicale, Attaque) comptent donc normalement pour la 1ʳᵉ victoire du jour et les quêtes quotidiennes, exactement comme n'importe quelle victoire en ligne. Aucun correctif à apporter — c'était noté comme un gap par erreur, ce n'en est pas un.

### G8b — Substitution Monban en défense (2026-08-18) — décision F levée
- Généralise le pipeline réactif du lot G0 (trigger `pg_net` + Edge Function `monban-move`) au-delà de l'Invasion : `sql_a_executer/guild_events_g8b_monban_defense.sql` (exécuté via MCP) ajoute `online_games.is_monban_defense` (colonne = l'autorisation, posée uniquement par `guild_events_combat_tick`) ; réécrit `guild_events_combat_tick` pour que le forfait d'un défenseur (équipe B, `kind='attack'`) marque `guild_event_participants.plays_as_monban=true` + `online_games.is_monban_defense=true` + **force `ready_black=true`** (sans ça le client de l'attaquant reste bloqué en `markReadyAndWaitForOpponent()`, qui sonde `ready_black` toutes les 500ms et ne démarre jamais le décompte) au lieu d'éliminer directement — la partie continue normalement, sans clôture immédiate ; nouveau trigger `guild_dispatch_monban_defense_trg` (séparé du trigger d'Invasion, gardes indépendantes) qui dispatche `monban-move` dès que le tour passe à Noir sur une partie substituée ; `guild_event_state` étendu (`plays_as_monban` par participant, `is_monban_defense` sur `current_match`).
- Edge Function `monban-move` (dashboard Supabase, version 2) : élargit la garde d'entrée de `is_invasion=true` à `(is_invasion=true OR is_monban_defense=true)`, sépare la logique spécifique à l'Invasion (contrôle `invasion_requests.status='monban'`, `skillRating` depuis `invasion_defender_id`, appel à `invasion_resolve` en fin de partie) de celle du Combat de guilde (aucun second contrôle nécessaire — `is_monban_defense` EST déjà l'autorisation ; `skillRating` depuis `black_player_id` ; aucun appel de résolution, `guild_events_combat_tick` détecte lui-même `status='finished'` au tick suivant, code déjà écrit depuis G4).
- `index.html` : indicateur 🤖 sur le nœud du joueur substitué dans l'arbre Tekken (`buildGuildEventTreeSvg`/`nodeSvg`) et bandeau dédié dans le bloc « duel en cours » (`openGuildEventRoster`) quand `current_match.is_monban_defense`.
- **Testé de bout en bout avec de vrais bots, pipeline complet** (le seul lot du chantier vérifié jusqu'au bout, pas seulement par simulation d'état) : attaque 3v3 déclarée → duel créé → défenseur jamais chargé (ready_black resté false) → 60s dépassées → substitution confirmée (`plays_as_monban=true`, `is_monban_defense=true`, `ready_black` forcé, partie NON close) → tour de l'attaquant simulé (écriture directe `turn='black'`) → **le trigger réactif a appelé l'Edge Function déployée, qui a réellement recalculé un coup légal et l'a écrit** (vérifié via `net._http_response` : HTTP 200, et `game_state.lastMoved` contient un déplacement cohérent avec les règles du jeu) → tour revenu à 'white' automatiquement. Duel forcé à la victoire du défenseur ensuite : le gain est bien crédité à l'HUMAIN substitué (`wins` incrémenté sur sa ligne `guild_event_participants`, pas une entité Monban séparée), l'attaquant correctement éliminé, le siège de l'équipe A avance normalement — la mécanique de résolution existante n'a nécessité AUCUNE modification pour gérer ce cas. Nettoyage complet effectué après test.
- Portée assumée (cohérente avec le forfait déjà en place depuis G4) : ne couvre que « jamais chargé la partie dans les 60s », pas un décrochage en cours de partie déjà commencée — construire la détection d'un abandon mi-partie serait un chantier de présence/heartbeat à part entière, hors décision M telle qu'implémentée jusqu'ici.

**🏁 Le chantier Combat de guilde (G0→G8b) est fonctionnellement complet, sans
limitation connue restante.** Les trois systèmes (Tournoi interne,
Confrontation amicale, Attaque de guilde) partagent le même moteur, testés
bot par bot, prêts pour un test à 2 comptes humains — voir
`docs/TESTS_MANUELS_COMBAT_GUILDE.md` pour ce qui reste à vérifier en
navigateur.
