...
- 🔴 **Limitation connue et documentée (pas cachée)** : décision F (« Monban remplace en DÉFENSE uniquement ») n'est **pas** construite dans ce lot — un défenseur absent au forfait (60s, décision M) est simplement ÉLIMINÉ, comme dans un Tournoi interne ou une Confrontation amicale, au lieu de voir Monban reprendre son duel. Construire cette substitution réactive demanderait de généraliser le trigger `pg_net`/Edge Function `monban-move` du lot G0 au-delà de l'Invasion (nouveau champ sur `online_games`, nouveau trigger scope, adaptation de la fonction) — un chantier à part entière, pas une extension triviale. **À faire avant toute mise en avant publique du mode Attaque**, sinon une guilde peut se faire piller pendant la nuit sans aucune résistance alors que la décision promettait le contraire.
- 🔴 **Gap transversal découvert en marge de ce lot, PAS spécifique à G8** : décision AB (« ces combats ne comptent ni pour la première victoire du jour, ni pour les quêtes quotidiennes ») n'est en réalité respectée par AUCUN des trois systèmes (G4 Tournoi interne, G6 Confrontation amicale, G8 Attaque) — `recordDailyFirstWin()` et le suivi de quêtes se déclenchent côté client sur TOUTE victoire (`playerWon`), sans filtrer `is_guild_event`. Non corrigé dans cette passe (le point d'accroche est dans le flux de fin de partie partagé par TOUS les modes de jeu, un changement à risque à ne pas faire à l'aveugle en fin de session) — à corriger dans un lot dédié, probablement en excluant `is_guild_event=true` côté serveur (RPC `record_daily_first_win`) plutôt que côté client.

**🏁 Le chantier Combat de guilde (G0→G8) est fonctionnellement complet.** Les
trois systèmes (Tournoi interne, Confrontation amicale, Attaque de guilde)
partagent le même moteur, testés bot par bot, prêts pour un test à 2 comptes
humains — sous réserve des deux limitations connues ci-dessus (substitution
Monban en défense, filtrage AB des quêtes quotidiennes), à traiter avant mise
en avant publique plutôt qu'en bloquant la sortie de ce chantier.
