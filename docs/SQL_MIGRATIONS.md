...
| ✅ | `guild_events_g8_attack.sql` | Exécuté via MCP le 2026-08-18. **Dernier lot du chantier Combat de guilde** (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions Q/R/V/W/X/AA/AD/AG/AJ) : colonnes `guilds.last_attack_at`/`last_attacked_at` ; `guild_attack_candidates` (ciblage par tranche d'Elo, patron `invasion_candidates`) ; `guild_attack_declare` (pas d'acceptation requise, cooldowns 7j/4j avec bypass admin) ; `guild_attack_cancel` (attaquant seul, ≤1h avant, transfert 5 🐉) ; `guild_event_cancel` réécrite pour rejeter `kind='attack'` ; `guild_events_checkin_tick` réécrite (pénalités Q/X) ; `guild_events_combat_tick` réécrite (résolution kind-aware, notifie les deux guildes pour friendly/attack — corrige un trou du lot G6 — vol de 10 🐉 + Elo plein poids pour l'attaque). Testé de bout en bout par impersonation : formule Elo vérifiée EXACTE sur un combat 4v4 réel (+16/-16), vol de Ryu, cooldowns (attaquant/défenseur/bypass admin/busy), annulation, décisions Q et X — tous confirmés. 🔴 Deux limitations documentées dans le roadmap (non bloquantes, à traiter avant mise en avant publique) : substitution Monban en défense non construite (décision F), et un gap transversal préexistant (G4/G6/G8) où la 1ʳᵉ victoire du jour/quêtes ne filtrent pas `is_guild_event` (décision AB). |
