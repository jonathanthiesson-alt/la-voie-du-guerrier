...
| ✅ | `invasion_reactive_monban.sql` | Exécuté via MCP le 2026-08-17. Lot G0 (`docs/ROADMAP_GUILD_BATTLE.md` § 3) : active l'extension `pg_net`, crée le trigger `invasion_dispatch_monban_trg` sur `online_games` (`AFTER UPDATE OF turn`) qui appelle l'Edge Function `monban-move` de façon asynchrone dès qu'une partie d'invasion active passe au tour de Monban. Ne remplace pas `scripts/bot-army.mjs` (`driveInvasions`), vient l'accélérer (latence visée < 1s au lieu du cron 10 min). Edge Function `monban-move` déployée séparément (voir `docs/ROADMAP_GUILD_BATTLE.md` § 3 pour le détail et les nuances de test). |
| ✅ | `guild_journal.sql` | Exécuté via MCP le 2026-08-17. Lot G1 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décision U) : table `guild_journal` + RLS (lecture réservée aux membres) + `guild_journal_log()` interne (EXECUTE révoqué pour authenticated) + `guild_journal_list()`. Réécrit `guild_create`/`guild_join`/`guild_approve`/`guild_leave`/`guild_kick`/`guild_set_member_rank`/`guild_challenge`/`guild_challenge_respond`/`guild_challenges_cleanup` pour y journaliser fondation, arrivées, départs, exclusions, changements de grade, déclarations et résolutions de Défi inter-guildes. 🔴 **Corrige au passage un bug de production réel découvert pendant le test** : `guild_join`/`guild_approve` inséraient encore `role='member'`, une valeur rejetée par la contrainte `guild_members_role_check` posée par `guild_ranks.sql` (déjà exécuté) — **plus personne ne pouvait rejoindre une guilde** depuis son exécution, ni en direct ni via validation du chef (exception Postgres brute affichée comme « Erreur. » générique côté client). Corrigé en `role='g4'`. Testé de bout en bout par impersonation (`request.jwt.claims`) sur un compte bot jetable : création, jointure, changement de grade, lecture RLS, exclusion, départ/dissolution en cascade — tous vérifiés. |
| ✅ | `guild_events_g2.sql` | Exécuté via MCP le 2026-08-17. Lot G2 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions N/S/A) : tables `guild_events`/`guild_event_participants` (schéma commun aux futurs `friendly`/`attack`, seul `internal` utilisable ici) + RLS + RPC `guild_event_create`/`guild_event_register`/`guild_event_unregister`/`guild_event_cancel`/`guild_event_list_mine` + tick `guild_events_registration_tick` (pg_cron, 20s, clôture les inscriptions à l'heure dite et notifie). Un seul Tournoi interne à la fois par guilde (décision S), Elo figé à l'inscription (décision A). Testé de bout en bout (création, inscription, contrainte « un seul à la fois », clôture automatique par le cron, nettoyage) sur une guilde jetable. |
| ✅ | `guild_events_g3.sql` | Exécuté via MCP le 2026-08-17. Lot G3 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions A/K/L/O) : RPC `guild_event_autobalance_teams` (répartition en serpentin par Elo décroissant + ordre de passage croissant par équipe), `guild_event_move_player` (glisser-déposer du chef, réordonne toujours l'Elo), `guild_event_checkin`, `guild_event_state` (lecture détaillée) + tick `guild_events_checkin_tick` (pg_cron, 20s : ouvre le check-in 5 min avant le début, puis à l'heure dite retire les absents, renumérote les sièges et passe l'événement en `running`, ou l'annule si une équipe se retrouve vide). Testé de bout en bout avec 6 bots jetables et des Elo distincts (répartition vérifiée équilibrée à 100 points près, check-in partiel, retrait des 2 absents, renumérotation sans trou, statut final `running`). |
| ✅ | `guild_events_g4.sql` + `guild_events_g4_state.sql` | Exécutés via MCP le 2026-08-17. Lot G4 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions I/J/M/S — « le cœur du mode ») : colonne `guild_events.winner_team`, colonnes `online_games.is_guild_event`/`guild_event_id`/`guild_event_match_id`, table `guild_event_matches` + RLS, tick `guild_events_combat_tick` (pg_cron, 20s) qui enchaîne réellement les duels façon Tekken Team Battle — crée le duel courant (partie en ligne normale, `ranked=false`), résout le duel fini (élimine le perdant, avance le siège de son équipe, calcule le streak du champion), respecte la pause de 30s entre deux duels, applique un forfait après 60s si un des deux joueurs n'a jamais chargé la partie (sans Monban, décision F — réservé à la défense d'attaque de guilde), et clôt le tournoi (aucune récompense, décision S) quand une équipe est vidée. `guild_event_state` étendu avec le duel courant et l'historique des duels. 🔴 **Bug réel trouvé et corrigé pendant le test** : le calcul du streak remontait par erreur les tentatives de duel ABANDONNÉES (forfait des deux côtés, `winner` null) comme « duel précédent », cassant la série du vrai champion dès qu'un abandon s'intercalait — corrigé en excluant `winner is null` de la recherche. Testé de bout en bout avec 6 bots jetables sur un tournoi 3v3 complet (3 duels réels + 2 tentatives abandonnées volontairement observées, élimination progressive de l'équipe perdante, victoire d'équipe, notifications et journal corrects). |
| — | *(aucun script)* | Lot G5 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions B/C) : purement client, aucune migration SQL. Arbre Tekken en SVG (`buildGuildEventTreeSvg()`) construit à partir des données déjà exposées par `guild_event_state` (G4). |
| ✅ | `guild_events_g6_friendly.sql` | Exécuté via MCP le 2026-08-17. Lot G6 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décision T) : ajoute `pending_acceptance` au statut de `guild_events`, RPC `guild_event_propose_friendly`/`guild_event_respond_friendly`/`guild_event_pending_for_me`, réécrit `guild_event_register` (équipe fixée par appartenance de guilde hors `internal`), `guild_event_cancel` (chef de l'une ou l'autre guilde), `guild_event_list_mine` (inclut guild_b), `guild_events_registration_tick` (ordre par Elo auto hors `internal`), `guild_events_checkin_tick` (comptage ≥3/équipe, décision O, applicable aussi au Tournoi interne), `guild_event_state` (kind + noms des deux guildes). Testé de bout en bout par impersonation sur deux guildes jetables (proposition → acceptation → inscription → répartition automatique par guilde d'origine → ordre par Elo → check-in → lancement → premier duel créé par le moteur G4 sans modification). Nettoyé après test. |
| ✅ | `guild_events_g7_elo.sql` | Exécuté via MCP le 2026-08-17 (3 migrations : colonne + vue + fonction Elo + `guild_challenges_cleanup`, puis `get_my_guild()`, puis `guild_roster()`). Lot G7 (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions AE/AF/AJ/AK/AL) : colonne `guilds.guild_elo` (défaut 1200) ; `v_guild_leaderboard` **DROP puis recréée** (colonne ajoutée au milieu, `create or replace view` aurait échoué en 42P16) triant sur `guild_elo desc` au lieu de `ryu_total` (le Ryu redevient une trésorerie pure) ; fonction interne `guild_elo_apply_result()` (formule Elo standard, marge = écart de score, poids paramétrable — 0.5 pour le Défi guilde 48h dès aujourd'hui, 1.0 réservé à l'Attaque de guilde du lot G8) ; `guild_challenges_cleanup` branchée dessus. Formule vérifiée isolément (Elo égaux, score 30-5 → +11/-11 en poids plein, +6/-6 en poids moitié). |
| ✅ | `guild_events_g8_attack.sql` | Exécuté via MCP le 2026-08-18. **Dernier lot du chantier Combat de guilde** (`docs/ROADMAP_GUILD_BATTLE.md` § 7, décisions Q/R/V/W/X/AA/AD/AG/AJ) : colonnes `guilds.last_attack_at`/`last_attacked_at` ; `guild_attack_candidates` (ciblage par tranche d'Elo, patron `invasion_candidates`) ; `guild_attack_declare` (pas d'acceptation requise, cooldowns 7j/4j avec bypass admin) ; `guild_attack_cancel` (attaquant seul, ≤1h avant, transfert 5 🐉) ; `guild_event_cancel` réécrite pour rejeter `kind='attack'` ; `guild_events_checkin_tick` réécrite (pénalités Q/X) ; `guild_events_combat_tick` réécrite (résolution kind-aware, notifie les deux guildes pour friendly/attack — corrige un trou du lot G6 — vol de 10 🐉 + Elo plein poids pour l'attaque). Testé de bout en bout par impersonation : formule Elo vérifiée EXACTE sur un combat 4v4 réel (+16/-16), vol de Ryu, cooldowns (attaquant/défenseur/bypass admin/busy), annulation, décisions Q et X — tous confirmés. (La décision AB — exclure ces combats de la 1ʳᵉ victoire du jour/quêtes — a été annulée par Wurmz le 2026-08-18 : « une victoire est une victoire ». Ce n'était pas un gap, c'est désormais le comportement voulu.) |
| ✅ | `guild_events_g8b_monban_defense.sql` | Exécuté via MCP le 2026-08-18 (2 migrations : colonne + tick + trigger, puis `guild_event_state`). Lève la limitation décision F laissée par G8 : colonne `online_games.is_monban_defense` ; `guild_events_combat_tick` réécrite (forfait d'un défenseur d'attaque → substitution Monban au lieu d'élimination, `ready_black` forcé pour débloquer le client de l'attaquant) ; nouveau trigger `guild_dispatch_monban_defense_trg` (pg_net) séparé de celui de l'Invasion. Edge Function `monban-move` mise à jour en parallèle (dashboard, version 2) : garde élargie `is_invasion OR is_monban_defense`. **Seul lot du chantier testé bout en bout avec le VRAI pipeline réactif** (pas une simulation d'état) : substitution confirmée, trigger→Edge Function→coup légal réellement joué (`net._http_response` HTTP 200), gain crédité au bon humain à la résolution. Le chantier Combat de guilde (G0→G8b) n'a plus de limitation connue. |
| ✅ | `monban_daily_training.sql` | Exécuté via MCP le 2026-08-18. `monban_mark_trained()` (`monban_core.sql`) ne posait qu'un horodatage, sans limite ni effet — **drop + recréation** (le type de retour change void→jsonb, `create or replace` aurait échoué en 42P13). Donne un vrai effet au rituel quotidien (décision O, `docs/ROADMAP_PUZZLE_INVASION.md`) : +2 skillRating (plafonné à 100), une fois par jour calendaire (`current_date`, même convention que `record_daily_first_win`), avec bypass illimité pour Wurmz/Musashi (`is_admin_user()`, pour les tests). Rappel : Monban apprend déjà de TOUTES les parties en ligne via `monban_learn_from_game` (hook existant dans `endGame()`, +3/-1 par partie) — ce rituel est un bonus en plus, pas le seul mécanisme d'apprentissage. Testé par impersonation : 1er appel +2 confirmé, 2e appel bloqué (`already_trained`), bypass admin vérifié (deux appels consécutifs tous deux acceptés). |
